...
function cshero_post_nav() {
	...
	<nav class="navigation post-navigation clearfix" role="navigation">
		<div class="nav-links">
			<?php
				previous_post_link( '<div class="nav-previous">%link</div>', '<span class="btn btn-primary-alt">'.__( 'PREVIOUS POST', THEMENAME ).'</span>' );
				next_post_link(     '<div class="nav-next">%link</div>',     '<span class="btn btn-primary-alt">'.__( 'NEXT POST', THEMENAME ).'</span>' );
			?>
		</div><!-- .nav-links -->
	</nav><!-- .navigation -->
	<?php
...
function cshero_posted_on() {
	...
	$posted_on = sprintf(
		__( 'Posted on %s', 'post date', THEMENAME ),
		'<a href="' . esc_url( get_permalink() ) . '" rel="bookmark">' . $time_string . '</a>'
	);

	$byline = sprintf(
		__( 'by %s', 'post author', THEMENAME ),
