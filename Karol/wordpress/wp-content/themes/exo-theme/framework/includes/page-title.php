...
/** values */
$title_class = $animation = $data_parallax = $subheader_text = $breadcrumb_text = $breadcrumb_padding = $pagetitle_content = $bg_overlay = $breadcrumb_content = '';
/** render options */
...
if(is_page() && isset($post->ID)){
    ...
    $breadcrumb_padding = get_post_meta($post->ID, 'cs_breadcrumb_padding', true);
...
if($breadcrumb){
    $breadcrumb_content = '<div id="cs-breadcrumb-wrapper" style="padding:'.$breadcrumb_padding.';"';
...
<section class="cs-content-header<?php if(is_sticky()){ echo " page-title-sticky"; } ?>">
    <div id="cs-page-title-wrapper" class="cs-page-title stripe-parallax-bg<?php echo esc_attr($title_class); ?><?php if(!empty($subheader_text)) {echo ' sub-post-on ';} ?>" <?php echo esc_attr($data_parallax); ?>>
    	<div class="container">
