...
if ($masonry_filter && count($categories) > 0) {	?>
			<div class="filter_outer">
				<h3><?php _e('CATEGORY', THEMENAME); ?></h3>
				<div class="filter_holder">
					<ul>
						<li class="filter active" data-filter="*"><span class="btn btn-primary-border btn-xs"><?php _e('All', THEMENAME); ?></span></li>
						<?php foreach ($categories as $category) { ?>
							 <li class="filter" data-filter="<?php echo ".category-" . $category->slug; ?>"><span class="btn btn-primary-border btn-xs"><?php echo ''.$category->name; ?></span></li>
						<?php } ?>
					</ul>
				</div>
			</div>
