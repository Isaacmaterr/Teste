...
function cshero_css(){
	...
	wp_register_style('vc-linecons-icons', get_template_directory_uri().'/css/vc_linecons_icons.css', array(), '4.1.0');
	...
	//wp_enqueue_style('icon7stroke', get_template_directory_uri() . '/css/pe-icon-7-stroke.css', array(), '1.1.0');
	...
	wp_enqueue_style( 'animations-column', get_template_directory_uri() . "/css/animations-column.css", array(), '1.0.0');
	if(class_exists('WooCommerce')){
		wp_enqueue_style( 'woocommerce', get_template_directory_uri() . "/css/woocommerce.css", array(), '1.0.0');
	}
	/*end prefix*/
	wp_enqueue_style( 'style', get_template_directory_uri() . "/style.css", array(), '1.0.0');
	wp_enqueue_style( 'linecons', get_template_directory_uri() . "/css/vc_linecons_icons.css", array(), '1.0.0');
	wp_enqueue_style( 'typicons', get_template_directory_uri() . "/css/typicons.css", array(), '1.0.0');
	wp_enqueue_style( 'openiconic', get_template_directory_uri() . "/css/vc_openiconic.css", array(), '1.0.0');
	wp_enqueue_style( 'entypo', get_template_directory_uri() . "/css/vc_entypo.css", array(), '1.0.0');

    if(cshero_getHeader()=='v4'){
        wp_enqueue_style('header-center', get_template_directory_uri().'/css/header-center.css',array(), '1.0');
    }
    /** Dynamic */
    $page_id = '';
...
function cshero_js(){
    ...
    wp_enqueue_script( 'exosameheight', get_template_directory_uri().'/js/exo-sameheight.js',array(), '1.0.0', TRUE);
    wp_enqueue_script( 'animation-column', get_template_directory_uri().'/js/animation-column.js',array(), '1.0.0', TRUE);
    wp_enqueue_script( 'jquery-direction', get_template_directory_uri().'/js/jquery.direction.js',array(), '1.0.0', TRUE);
    wp_enqueue_script( 'matchHeight', get_template_directory_uri().'/js/jquery.matchHeight.js',array(), '1.0.0', TRUE);
    wp_enqueue_script( 'jquery-nav', get_template_directory_uri().'/js/jquery.nav.js',array(), '1.0.0', TRUE);
    wp_enqueue_script( 'enscroll', get_template_directory_uri().'/js/enscroll.js',array(), '1.0.0', TRUE);
    ...
    if(is_page() && !empty($post->ID)){
        ...
        if($one_page){
            ...
            wp_enqueue_script( 'jquery.singlePageNav', get_template_directory_uri().'/js/jquery.singlePageNav.js', array(), '3.0.0', TRUE);
...
function cs_socials_share($url,$image='',$title='',$comment_link=''){
	...
	<div class="post-share">
		...
		<a target="_blank" href="https://twitter.com/home?status=<?php echo __('Check out this article',THEMENAME);?>:%20<?php echo esc_attr($title);?>%20-%20<?php echo esc_url($url);?>"><span class="share-box"><i class="fa fa-twitter"></i></span></a>
...
add_image_size('woo-thumb', 115, 145, true);

add_action('widgets_init', 'cshero_register_sidebar');

function cshero_register_sidebar(){
...
if(function_exists('register_sidebar')) {
    ...
    register_sidebar(array(
        'name' => 'Woocommerce Cart Header',
        'id' => 'woocommerce-cart-header',
        'before_widget' => '<div id="%1$s" class="cart-widget-col %2$s">',
    	'after_widget' => '<div style="clear:both;"></div></div>',
    	'before_title' => '<h3 class="wg-title"><span>',
    	'after_title' => '</span></h3>',
    ));
...
if(!function_exists('woo_add_custom_general_fields')){
    function woo_add_custom_general_fields()
    {
        ...
        woocommerce_wp_text_input(array(
            ...
            'label' => __('New Text', THEMENAME),
            'value' => $_spectrum_new_item,
            'desc_tip' => 'true'
        ));
        woocommerce_wp_text_input(array(
            'id' => '_spectrum_best_sale',
            'label' => __('Bestseller Text', THEMENAME),
...
add_action('wp_ajax_woo_load_more', 'woo_load_more_callback');
add_action('wp_ajax_nopriv_woo_load_more', 'woo_load_more_callback');

function woo_load_more_callback() {
    
    $posts_per_page = !empty($_REQUEST['posts_per_page']) ? $_REQUEST['posts_per_page'] : 10;
...
add_action('init', 'woo_wishlists');

function woo_wishlists() {
        
    if(!isset($_GET['wl_pid'])) return ;
    ...
    // get array ids from SESSION.
    if(isset($_SESSION['wl_pids'])){
        
        $wl_pids = $_SESSION['wl_pids'];
    } 
    
    // id not in array.
    if(!in_array($_GET['wl_pid'], $wl_pids)){
        
        $wl_pids[] = $_GET['wl_pid'];
        
        // add SESSION.
        $_SESSION['wl_pids'] = $wl_pids;
    }
}
