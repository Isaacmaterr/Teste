...
/* Arrow Slider */
$cl_arrow_slider = '';
if ($arrow_slider == 'true') {
    $cl_arrow_slider = 'bullets-active';
}
/* Row Arrow */
$html_row_arrow = '';
$class_row_arrow = '';
...
/* Same Height */
$class_same_height = '';
if($exo_same_height == 'yes') {
    $class_same_height .= 'exo-same-height';
}
/* End Row Arrow */
/* row class */
    $lax_class = $lax_layer1 ? 'lax-active' : '';
    $row_sc_class = $enable_row_sc ? 'header-wrapper' : '';
    $css_class = apply_filters( VC_SHORTCODE_CUSTOM_CSS_FILTER_TAG,'wpb_row clearfix '.$cl_arrow_slider.' ' .$class_same_height. ' ' .$class_row_arrow. ' '.$engle_position.' '.$engle_position_style2.' '.$lax_class.' '. get_row_css_class() . $el_class . $responsive . $main_full_width . vc_shortcode_custom_css_class( $css, ' ' ), $this->settings['base'], $atts );
/* Link Color */
    global $link_style;
    $uqid = uniqid();
...
/* Row Curve */
if($enable_curve){
    $pen_curve = 'M0 0 C50 100 50 100 100 0  L100 100 0 100';
    $output .= '<svg xmlns="http://www.w3.org/2000/svg" width="100%" viewBox="0 0 100 100" version="1.1" preserveAspectRatio="none" height="'.$curve_height.'" class="engle-curve curve-position-top" style="fill:'.$curve_color.'"><path stroke-width="0" d="'.$pen_curve.'"/></svg>';
}
if($enable_curve2){
    $pen_curve2 = 'M0 0 C50 100 50 100 100 0  L100 100 0 100';
    $output .= '<svg xmlns="http://www.w3.org/2000/svg" width="100%" viewBox="0 0 100 100" version="1.1" preserveAspectRatio="none" height="'.$curve_height2.'" class="engle-curve curve-position-bottom" style="fill:'.$curve_color2.'"><path stroke-width="0" d="'.$pen_curve2.'"/></svg>';
}
...
/*** BG Color ***/
$output .= '<div class="wpb-row-inner">';
